...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  5 01:50:59 2022
...
def Train_AMIL(datasets, fold, args, trainFull = False):
    ...
    model = Att_MIL(**model_dict, instance_loss_fn=instance_loss_fn)
    ...
    for epoch in range(args.max_epochs):       
        ...
        Train_loop_CLAM(epoch = epoch, model = model, loader = train_loader, optimizer = optimizer, n_classes = args.n_classes,
                        bag_weight = args.bag_weight,loss_fn =  loss_fn)  
        if args.early_stopping:
            stop = validate_CLAM(fold = fold, epoch = epoch, model = model, loader = val_loader, n_classes = args.n_classes, early_stopping = early_stopping,
                             loss_fn = loss_fn, results_dir = args.output_dir)
    ...
    if (args.early_stopping) and (not trainFull):
        model.load_state_dict(torch.load(os.path.join(args.output_dir,"bestModelFold_" + str(fold))))
    if not trainFull:    
        patient_results, test_error, test_auc = Summary(model = model, loader = test_loader, n_classes = args.n_classes)
...
def Summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label, _) in enumerate(loader):
        ...
        patient_results.update({fileName: {'case_id': patient,'slide_id': fileName, 'prob': probs, 'label': label.item()}})
        error = utils.calculate_error(Y_hat, label)
        test_error += error
...
def Train_model_Classic(model, trainLoaders, args, valLoaders = [], criterion = None, optimizer = None, fold = False):    
    ...
    for epoch in range(args.max_epochs):
        ...
        if valLoaders:
            ...
            ckpt_name = os.path.join(args.output_dir, "bestModelFold" + fold)
            early_stopping(epoch, val_loss, model, ckpt_name = ckpt_name)
            if early_stopping.early_stop:
                print('-' * 30)
                print("The Validation Loss Didn't Decrease, Early Stopping!!")
