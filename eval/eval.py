...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 02:03:30 2022
...
def CalculatePatientWiseAUC(resultCSVPath, args, reportFile, foldcounter = None, attMil = False):
    
    data = pd.read_csv(resultCSVPath)
    patients = list(set(data['PATIENT']))
    keys = list(args.label_dict.keys())
    yProbDict = {}    
    for index, key in enumerate(keys):
        ...
        for patient in patients:
            ...
            yTrueLabelList.append(utils.get_key_from_value(args.label_dict, data_temp['yTrue'][0]))
        ...
        fpr, tpr, thresholds = metrics.roc_curve(yTrueList, yProbList, pos_label = args.label_dict[key])
        if foldcounter:            
...
def MergeResultCSV(resultsPath, results, attMil = False):
